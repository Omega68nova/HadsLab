...
def index(request):
    try :
        filter=request.POST['filtro']
    except:
        filter='default'
    latest_question_list = Question.objects.order_by('-pub_date')
    topicList = Question.objects.values_list('topic_text',flat=True).distinct()
    template = loader.get_template('polls/index.html')
    context = {'title':'Lista de preguntas de la encuesta', 'latest_question_list': latest_question_list,'topicList':topicList,'filter':filter}
...
def results(request, question_id, choice_id):
    ...
    if(choice.IsCorrect==True):
        ...
        msg='x'
    return render(request, 'polls/results.html', {'title':'Resultados de la pregunta:','question': question,'message':msg})
...
def choice_add(request, question_id):
        ...
        return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ question1.question_text,'form': form,'error_message': mensaje})
